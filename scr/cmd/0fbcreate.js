  ...
  start: async function ({ api, text, react, event, reply }) {
    try {
      ...
      if (isNaN(amount) || amount <= 0) {
        return reply("❌ Invalid number of accounts requested. Please specify a positive integer.");
      }

      reply(`🚀 Creating ${amount} Facebook account(s)... Please wait.`);
      ...
      for (let i = 0; i < amount; i++) {
        ...
        if (account) {
          ...
          if (regData) {
            ...
            reply(`⚠️ Failed to register account: ${account.email}`);
          }
        } else {
          reply(`⚠️ Failed to create email for account ${i + 1}.`);
        }
      }
      ...
      if (accounts.length > 0) {
        ...
        reply("❌ No accounts were created successfully.");
      }
    } catch (error) {
      console.error(error);
      reply("❌ An error occurred while creating Facebook accounts. Please try again.");
    }
  },
